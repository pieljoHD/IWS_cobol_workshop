...
FILE-CONTROL.
    SELECT BUCHUNGEN ASSIGN TO "BUCHUNGEN.DAT"
        ...
        FILE STATUS IS FSIN.
    SELECT FEHLER-LOG ASSIGN TO "buchungen-fehler.dat"     *> <<< ÄNDERUNG ggü. vorher (Dateiname)
        ORGANIZATION IS LINE SEQUENTIAL
...
FD FEHLER-LOG.
01 FEHLER-RECORD               PIC X(600).               *> <<< ÄNDERUNG ggü. vorher (Länge)
...
*> Eingabefelder: Vorname Nachname Stunden Gehalt Geburtsdatum
...
MAIN.
    ...
    PERFORM UNTIL EOF-FLAG = "Y"
        ...
        OR LENGTH OF FUNCTION TRIM(LINE-TRIM) = 0
            *> Leere Zeile -> nächste Iteration
            CONTINUE
        END-IF

        PERFORM PARSE-FIELDS
        PERFORM BUILD-NAME-DISPLAY
...
OPEN-FILES.
    ...
    END-IF
    IF FSERR NOT = "00"
        DISPLAY "Fehler beim Oeffnen FEHLER.LOG, Status=" FSERR   *> Hinweis: Text nennt "FEHLER.LOG".
        *> <<< ÄNDERUNG ggü. vorher: Wenn du es konsistent haben willst, formuliere z. B.:
        *> DISPLAY "Fehler beim Oeffnen buchungen-fehler.dat, Status=" FSERR
        GOBACK
    END-IF
    EXIT PARAGRAPH.
...
PARSE-FIELDS.
    MOVE SPACES TO WS-VORNAME WS-NACHNAME WS-STUNDEN-RAW WS-GEHALT-RAW WS-GEBURT-RAW
    UNSTRING LINE-TRIM
...
*> ================================================================
*> #6 Validierungen (deine Logik 1:1 übernommen)
*> ================================================================
PRUEFE-NAME.
...
*> ================================================================
*> #7 Logging (Semikolon-Format)
*> ================================================================
LOG-ERROR.
    MOVE SPACES TO FEHLER-RECORD
    STRING
        FUNCTION TRIM(LINE-TRIM)
        " ; "                                              *> <<< ÄNDERUNG ggü. vorher (neues Format)
        "Fehler: " FUNCTION TRIM(ARG-FELD)                 *> <<< ÄNDERUNG ggü. vorher (neues Format)
        " - "      FUNCTION TRIM(ARG-MSG)                  *> <<< ÄNDERUNG ggü. vorher (neues Format)
        INTO FEHLER-RECORD
    END-STRING
    WRITE FEHLER-RECORD
